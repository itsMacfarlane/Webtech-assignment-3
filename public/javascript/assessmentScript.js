...
function initializeHeader() {
	...
	img.setAttribute('src','https://www.w3.org/html/logo/downloads/HTML5_Logo_512.png');
...
function createFooterButton(className, id, buttonText) {
	var elem = document.getElementsByClassName("footer__button-group footer__menu")[0];
...
var dropDownFooter = class {
	constructor(labelClassName, labelName, selectClassName, selectId, options, input, locationName) {
...
var Question = class {
	...
	checkAnswer(title, rightInput, checked) {
		...
		var input = document.querySelector('input[name=' + CSS.escape(title) + ']' + checked);
...
class selectQuestion extends Question{
	...
	initializeFieldMC() {
		...
		div.appendChild(createInput(this.title, this.possibleAnswers[0], "radio", 0));
		div.appendChild(document.createTextNode(this.possibleAnswers[0]));
		div.appendChild(br1);

		div.appendChild(createInput(this.title, this.possibleAnswers[1], "radio", 0));
		div.appendChild(document.createTextNode(this.possibleAnswers[1]));
		div.appendChild(br2);

		div.appendChild(createInput(this.title, this.possibleAnswers[2], "radio", 0));
		div.appendChild(document.createTextNode(this.possibleAnswers[2]));
		div.appendChild(br3);

		div.appendChild(createInput(this.title, this.possibleAnswers[3], "radio", 0));
		div.appendChild(document.createTextNode(this.possibleAnswers[3]));

		div.addEventListener("change", this.checkAnswer.bind(this, this.title, this.correctAnswer, 1));
	...
	initializeFieldDropDown() {
		...
		selectMenu.addEventListener("input", this.checkAnswerDropDown.bind(this, this.title, this.correctAnswer));
	}
...
class FITBQuestion extends Question{
	...
	initializeFieldFITB() {
		...
		var inputField = createInput(this.title, null, "text", this.placeHolder);
		field.appendChild(inputField);

		questionDiv.appendChild(field);;

		inputField.addEventListener("input", this.checkAnswer.bind(this, this.title, this.correctAnswer, 0));
	}
...
function createInput(name, value, type, placeHolder) {
	...
	var inputElem = document.createElement('input');
		Object.assign(inputElem, {
			...
			name: this.name,
			value: this.value});
	if (this.placeHolder) Object.assign(inputElem, {placeholder: this.placeHolder});
	return inputElem;
...
dropDownFooter1 = new dropDownFooter("select-element-menu__label", "Select tag:", "select-element-menu__select-tag", "selectTagMenu", ["body", "header", "footer", "article", "aside", "section"], false, "footer__select-element-menu footer__menu");
dropDownFooter2 = new dropDownFooter("select-element-menu__label", "Select Element:", "select-element-menu__select-item", "elementsOfTag", [], false, "footer__select-element-menu footer__menu");
dropDownFooter3 = new dropDownFooter("select-property-menu__label", "Select property:", "select-propert-menu__select-color-font", "selectedProperty", ["backgroundColor", "fontColor", "fontSize"], false, "footer__select-property-menu footer__menu");
dropDownFooter4 = new dropDownFooter("select-propert-menu__label", "Choose a color or font size:", "color-font-group__font", "fontSize" ,["xx-small", "x-small", "small", "medium", "large", "x-large", "xx-large"], true, "footer__select-property-menu footer__menu")

Q1 = new FITBQuestion("Q1", "In January of 2000, ______ became a standard together with HTML 4.01.", "XHTML", "Fill in the blank:");
Q2 = new selectQuestion("Q2", "In which country was the World Wide Web invented?", "Switzerland", ["Switzerland", "USA", "France", "UK"]);
Q3 = new selectQuestion("Q3", "What does the H in HTML stand for?", "Hyper", ["Hyper", "Horror", "Historic", "Hard", "Handy", "Hardware"]);
Q4 = new selectQuestion("Q4", "Who knows the most about the internet?", "Sergey Sosnovksy", ["Sergey Sosnovksy", "Bill Gates", "Tim Berners-Lee", "Jorrit Koerts"]);
Q5 = new FITBQuestion("Q5", "Paragraph. Enclose paragraphs in an article within ______ tags.", "<p>", "Fill in the blank:");


window.addEventListener('load', function(){
	initializeDiv("footer__webtech", "Webtech UU", "http://www.cs.uu.nl/education/vak.php?vak=INFOB2WT");
	initializeDiv("footer__navigation", null, null);
	initializeDiv("footer__terms", "Terms and conditions", "../resources/Terms.pdf");
	...
	createFooterButton("button-group__light-mode", "darkmodeToggle", "Toggle light mode");
	createFooterButton("button-group__count", "countSelectedElement", "count selected element");
	initializeHeader();
	initializeQuestionDiv();
	initializeUnorderedList("navigation__list", "upperList", "body__navigation");
	initializeUnorderedList("navigation__list", "lowerList", "footer__navigation");
